...
const SliderContentInner = ({
  ...
  return /*#__PURE__*/r__default["default"].createElement("div", _extends({}, props, {
    size: size,
    ref: innerRef
  }), children);
};
...
const Carousel = ({
  ...
  const [propsValues, setPropsValues] = r$1.useState({});
  const [currentIndex, setCurrentIndex] = r$1.useState(0);
  const [numberOfChildren, setNumberOfChildren] = r$1.useState(0);
  const [childrenSlides, setChildrenSlides] = r$1.useState([]);
  const [carouselSlideShow, setCarouselSlideShow] = r$1.useState(null);
  const [slideSize, setSlideSize] = r$1.useState(0);
  const [size, setSize] = r$1.useState([0, 0]); // let slideSize =
  //   size[0] >= 1960
  ...
  const useIsomorphicLayoutEffect = canUseDOM ? r$1.useLayoutEffect : r$1.useEffect;
  ...
  const forwardButtonRef = r$1.useRef(null);
  const currentIndexRef = r$1.useRef(currentIndex);
  ...
  const onNextClicked = () => {
    if (currentIndexRef.current != numberOfChildren - 1) setCurrentIndex(currentIndexRef.current + 1);else setCurrentIndex(slideSize - 1);
    ...
    if (auto) {
      setCarouselSlideShow(setInterval(() => {
        clickNext();
      }, slideShowDuration));
    }
  };
  ...
  const onPrevClicked = () => {
    if (currentIndexRef.current != slideSize - 1) setCurrentIndex(currentIndexRef.current - 1);else setCurrentIndex(numberOfChildren - 1);
  ...
  r$1.useEffect(() => {
    ...
    if (height) properties = { ...properties,
      height
    };
    if (width) properties = { ...properties,
      width
    };
    let slideSizeTemp = size[0] >= 1960 ? breakpoints?.extraLargeScreen || 8 : size[0] >= 1440 ? breakpoints?.largeScreen || 5 : size[0] >= 769 ? breakpoints?.laptop || 3 : size[0] >= 481 ? breakpoints?.tab || 2 : breakpoints?.phone || 1;
  ...
  return /*#__PURE__*/r__default["default"].createElement(CarouselContainerStyled, null, /*#__PURE__*/r__default["default"].createElement("div", {
    className: "secondary-container",
    style: { ...propsValues
    }
  }, /*#__PURE__*/r__default["default"].createElement("span", {
    className: `arrow left-arrow ${currentIndex === slideSize - 1 && !infinite && "hidden"} ${numberOfChildren === slideSize && !infinite && "hidden"} ${noControls && !infinite && "hidden"}`,
    onClick: () => onPrevClicked(),
    title: "Previous Slide"
  }, leftButton ? leftButton : /*#__PURE__*/r__default["default"].createElement("button", {
    className: `custom-arrow-btn`
  }, /*#__PURE__*/r__default["default"].createElement("span", {
    style: {
      fontSize: 25
    }
  }, "\u2039"))), /*#__PURE__*/r__default["default"].createElement("div", {
    className: "slider-content-container"
  }, /*#__PURE__*/r__default["default"].createElement(SliderContent, {
    currentIndex: currentIndex,
    setNumberOfSlides: setNumberOfSlides,
    setChildrenSlidesFunc: setChildrenSlidesFunc,
    size: size[0] >= 1960 ? breakpoints?.extraLargeScreen || 8 : size[0] >= 1440 ? breakpoints?.largeScreen || 5 : size[0] >= 769 ? breakpoints?.laptop || 3 : size[0] >= 481 ? breakpoints?.tab || 2 : breakpoints?.phone || 1
  }, children)), /*#__PURE__*/r__default["default"].createElement("span", {
    className: `arrow right-arrow ${currentIndex === numberOfChildren - 1 && !infinite && "hidden"} ${numberOfChildren == slideSize && !infinite && "hidden"} ${noControls && !infinite && "hidden"}`,
    ref: forwardButtonRef,
    onClick: () => onNextClicked(),
    title: "Next Slide"
  }, rightButton ? rightButton : /*#__PURE__*/r__default["default"].createElement("button", {
    className: `custom-arrow-btn`
  }, /*#__PURE__*/r__default["default"].createElement("span", {
    style: {
      fontSize: 25
...
const SliderContent = ({
  ...
  const [innerWidth, setInnerWidth] = r$1.useState(0);
  const childrenCount = r$1.Children.count(children);
  const ref = r$1.useRef(null);

  const getWidth = () => {
    if (ref.current) setInnerWidth(ref.current.offsetWidth);else setInnerWidth(0);
  };
  ...
  const props = {
    transform: () => {
      let transform = 0;
      if (currentIndex <= size - 1) transform = 0;else transform = innerWidth / size * (currentIndex - (size - 1));
      return transform.toString();
    },
    innerRef: ref,
    size: size
  };
  return /*#__PURE__*/r__default["default"].createElement(SliderContentStyled, props, children);
};
...
const Slide = ({
  ...
  const [size, setSize] = r$1.useState([0, 0]);
  const canUseDOM = typeof window !== "undefined";
  const useIsomorphicLayoutEffect = canUseDOM ? r$1.useLayoutEffect : r$1.useEffect;
  useIsomorphicLayoutEffect(() => {
    function updateSize() {
  ...
  return /*#__PURE__*/r__default["default"].createElement(SlideStyled, {
    className: "",
    size: size[0] >= 1960 ? breakpoints?.extraLargeScreen || 8 : size[0] >= 1440 ? breakpoints?.largeScreen || 5 : size[0] >= 769 ? breakpoints?.laptop || 3 : size[0] >= 481 ? breakpoints?.tab || 2 : breakpoints?.phone || 1
  }, children);
};
